...
def tenant_usage():
    ...
    token = session.get('token')
    if token!= None:
        tenant_list = get_tenant_list(token,hostname,keystone_port)
        tenant_admin_id = get_tenant_id(token,hostname,keystone_port,'admin')
        tenant_usage = {}
        all_tenant_usage = []
        for tenant in range(len(tenant_list['tenants'])):
            ...
            tenant_usage_detail = get_tenant_usage(tenant_admin_id,tenant_list['tenants'][tenant]['id'],token,hostname,nova_port)
